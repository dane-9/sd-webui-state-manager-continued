...
(function(sm: StateManager): void{
    ...
    sm.getNormalisedSortValue = function(value: any): string{
    ...
    sm.getNormalisedUISettings = function(settings: any){
        const normalised = {
            historySmallViewEntriesPerPage: entriesPerPage,
            favouritesSmallViewEntriesPerPage: entriesPerPage,
            collapseSmallViewAccordion: false,
    ...
    sm.getNormalisedStoredEntryFilter = function(filter: any){
    ...
    sm.syncEntryFilterControls = function(){
        ...
        const searchInput = <HTMLInputElement | null>sm.panelContainer.querySelector('.sd-webui-sm-entry-header .search-row input');
        const txt2imgCheckbox = <HTMLInputElement | null>sm.panelContainer.querySelector('#sd-webui-sm-filter-txt2img');
        const img2imgCheckbox = <HTMLInputElement | null>sm.panelContainer.querySelector('#sd-webui-sm-filter-img2img');
        const showSavedCheckbox = <HTMLInputElement | null>sm.panelContainer.querySelector('#sd-webui-sm-filter-favourites-checkbox');
        const sortSelect = <HTMLSelectElement | null>sm.panelContainer.querySelector('#sd-webui-sm-sort');
    ...
    sm.syncUISettingsControls = function(){
        ...
        const historySmallViewEntriesInput = <HTMLInputElement | null>sm.panelContainer.querySelector('#sd-webui-sm-settings-small-view-entries-history');
        const favouritesSmallViewEntriesInput = <HTMLInputElement | null>sm.panelContainer.querySelector('#sd-webui-sm-settings-small-view-entries-favourites');
        ...
        const showConfigTypeBadgeCheckbox = <HTMLInputElement | null>sm.panelContainer.querySelector('#sd-webui-sm-settings-show-config-type-badge');
    ...
    sm.getQuickConfigMenuStates = function(): State[]{
        if(!sm.memoryStorage?.entries?.data){
            return [];
        }

        return Object.values(sm.memoryStorage.entries.data)
            .filter((state: State) => (state.groups?.indexOf('favourites') ?? -1) > -1)
            .sort((a: State, b: State) => (Number(b.createdAt ?? 0) || 0) - (Number(a.createdAt ?? 0) || 0))
            .slice(0, 10);
    }

    sm.syncQuickConfigApplyButtonState = function(): void{
        const applyButton = <HTMLButtonElement | null>(sm.quickConfigApplyButton || null);
    ...
    sm.syncQuickConfigMenuSelectionState = function(): void{
        const container = <HTMLElement | null>(sm.quickConfigMenuContainer || null);
        ...
        const selectedStateKey = `${sm.quickMenuSelectedStateKey ?? ''}`;
        const quickConfigButtons = <NodeListOf<HTMLButtonElement>>container.querySelectorAll('.sd-webui-sm-quick-config-item');
    ...
    sm.ensureQuickConfigHoverTooltip = function(): HTMLElement{
        let tooltip = <HTMLElement | null>(sm.quickConfigHoverTooltip || null);
    ...
    sm.hideQuickConfigHoverTooltip = function(): void{
        const tooltip = <HTMLElement | null>(sm.quickConfigHoverTooltip || null);
    ...
    sm.showQuickConfigHoverTooltip = function(label: string, anchorElement: HTMLElement): void{
        ...
        const tooltip = <HTMLElement>sm.ensureQuickConfigHoverTooltip();
    ...
    sm.syncModalQuickControlsState = function(): void{
        const isModal = Boolean(sm.panelContainer?.classList.contains('sd-webui-sm-modal-panel'));
        const quickSaveButton = <HTMLButtonElement | null>(sm.quickSettingSaveButton || null);
        const quickApplyButton = <HTMLButtonElement | null>(sm.quickConfigApplyButton || null);
        const quickMenuContainer = <HTMLElement | null>(sm.quickConfigMenuContainer || null);
        const accordionToggleButton = <HTMLButtonElement | null>(sm.smallViewAccordionToggleButton || null);
    ...
    sm.applyQuickSelectedConfig = function(): void{
        const selectedStateKey = `${sm.quickMenuSelectedStateKey ?? ''}`;
    ...
    sm.renderQuickConfigMenu = function(): void{
        const container = <HTMLElement | null>(sm.quickConfigMenuContainer || null);
        const navControlButtons = <HTMLElement | null>(container?.parentElement || null);
        ...
        const quickConfigKeys = new Set<string>(quickConfigs.map((state: State) => `${state.createdAt ?? ''}`));
        ...
        for(const state of quickConfigs){
            const button = <HTMLButtonElement>sm.createElementWithClassList('button', 'sd-webui-sm-quick-config-item');
            ...
            const selectQuickConfig = (event: Event): void => {
            ...
            const showQuickConfigTooltip = (): void => {
                sm.showQuickConfigHoverTooltip?.(`${button.title ?? ''}`, button);
            };

            const hideQuickConfigTooltip = (): void => {
                sm.hideQuickConfigHoverTooltip?.();
            };
    ...
    sm.areBooleanMapsEqual = function(a: {[key: string]: boolean}, b: {[key: string]: boolean}): boolean{
        ...
        const getCheckboxState = (source: {[key: string]: boolean}, key: string): boolean => source?.hasOwnProperty(key) ? Boolean(source[key]) : true;
    ...
    sm.injectUI = function() {
        ...
        const quickSettingSaveButton = <HTMLButtonElement>sm.createElementWithInnerTextAndClassList('button', defaultQuickSettingSaveButtonText, 'sd-webui-sm-nav-save-button', 'sd-webui-sm-nav-save-current-config-button', 'lg', 'secondary', 'gradio-button', sm.svelteClasses.button);
        ...
        const showQuickSettingSaveButtonResult = (success: boolean) => {
        ...
        let navControlButtons: HTMLElement | null = null;
        const panelTabButtons: {[key: string]: HTMLButtonElement} = {};
        ...
        const setActivePanelTab = (tab: string, group?: Group) => {
        ...
        function createNavTab(label: string, tab: 'history' | 'favourites' | 'settings', group: Group | null, isSelected?: boolean){
            const button = <HTMLButtonElement>sm.createElementWithInnerTextAndClassList('button', label, sm.svelteClasses.tab);
        ...
        const quickConfigApplyButton = <HTMLButtonElement>sm.createElementWithInnerTextAndClassList('button', 'Apply Config', 'sd-webui-sm-nav-save-button', 'sd-webui-sm-nav-apply-config-button', 'lg', 'secondary', 'gradio-button', sm.svelteClasses.button);
        ...
        function createFilterToggle(type: string){
            return sm.createPillToggle(type, {title: `Show ${type} entries`}, `sd-webui-sm-filter-${type}`, sm.entryFilter.types.indexOf(type) > -1, (isOn: Boolean) => {
        ...
        showSavedConfigsToggle = sm.createPillToggle('Show Saved Configs', {title: "Show saved configs in history", id: 'sd-webui-sm-filter-favourites'}, 'sd-webui-sm-filter-favourites-checkbox', sm.entryFilter.showFavouritesInHistory, (isOn: boolean) => {
        ...
        const applySmallViewEntriesChange = (group: Group, input: HTMLInputElement): void => {
...
[]        but only when the resulting value is different from either [current UI value | current UI default]
    ...
    sm.saveLastUsedState = function(group: Group = 'history'): void{
        if(!sm.lastUsedState){
